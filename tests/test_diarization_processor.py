...
def test_parse_arguments():
    with patch('sys.argv', ['script.py', '--input-file', 'input.json', '--output-dir', 'output', '--debug']):
        args = parse_arguments()
        assert args.input_file == 'input.json'
        assert args.output_dir == 'output'
        assert args.input_dir is None

    with patch('sys.argv', ['script.py', '--input-dir', 'input_dir', '--output-dir', 'output', '--debug']):
        args = parse_arguments()
...
def test_main_single_file(mock_check_file_exists, mock_process_file):
    ...
    with patch('sys.argv', ['script.py', '--input-file', 'input.json', '--output-dir', 'output']):
        main()
...
def test_main_directory(mock_glob, mock_is_dir, mock_process_file):
    ...
    with patch('sys.argv', ['script.py', '--input-dir', 'input_dir', '--output-dir', 'output']):
        main()
...
@pytest.mark.parametrize("invalid_data", [
    ...
    {"jobId": "123", "status": "succeeded", "output": {"diarization": [{"invalid": "data"}]}},  # Missing required keys
    {"jobId": "123", "status": "succeeded", "output": {"diarization": [{"speaker": "SPEAKER_00", "start": "invalid", "end": 1.0}]}},  # Invalid start time
])
